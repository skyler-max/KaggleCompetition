...
#载入停用词表，把停用词表的词都放到stop_words这个列表里面
def load_stop_words(stop_word_file):
    ...
    for line in open(stop_word_file):
        if line.strip()[0:1] != "#":#第一个字符不为#也就是不是注释
...
def separate_words(text, min_word_return_size):
    ...
    """
    splitter = re.compile('[^a-zA-Z0-9_\\+\\-/]')#匹配所有的字母和数字 返回一个正则表达式
    words = []
    for single_word in splitter.split(text):
        current_word = single_word.strip().lower()#把所有字符串转为小写字母
        # leave numbers in phrase, but don't count as words, since they tend to invalidate scores of their phrases
        if len(current_word) > min_word_return_size and current_word != '' and not is_number(current_word):
            words.append(current_word)
    return words

#1.用标点符号（如半角的句号，问号，感叹号，逗号等）将一篇文档分为若干分句
...
#用停用词表作为分隔符将分句分为若干短语，将这些短语连接成一个字符串，并将这些短语正则化
...
def generate_candidate_keywords(sentence_list, stopword_pattern):
    ...
    for s in sentence_list:
        tmp = re.sub(stopword_pattern, '|', s.strip())#对于分句后的每个句子将候选词短语的正则用“|"代替
        phrases = tmp.split("|")
